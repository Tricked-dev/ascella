...
pub fn command() -> Command {
  CommandBuilder::new("emojis".into(), "Enable emoji urls.".into(), CommandType::ChatInput).build()
}
...
pub async fn execute(client: &Client, cmd: &ApplicationCommand, user: Users) -> Result<()> {
  ...
  let embed = create_embed()
    .title(format!("Turned emoji urls {}", if user.emojis.unwrap_or(false) { "off" } else { "on" }))
