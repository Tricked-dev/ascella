...
pub async fn post(req: &mut Request, res: &mut Response) {
  ...
  let user_token: String = req.get_header("x-user-token").unwrap_or_else(|| "none".to_owned());
  ...
  if user_id == -1 || user_token == "none" {
    ...
    if let Ok(data) = data {
      ...
      if let Some(file) = file {
        ...
        let content_type = file.headers().get("content-type").unwrap_or(default_content_type).to_str().unwrap();
        ...
        let img = create_image::exec(
          ...
          if data.emojis.unwrap_or(false) { random_emojis() } else { ran_str() },
        )
        .await
        .unwrap();
