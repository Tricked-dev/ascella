...
pub fn command(domain_options: impl IntoIterator<Item = (String, String)>) -> Command {
    CommandBuilder::new(
        "domain".into(),
        "Select a domain to be used when uploading.".into(),
        CommandType::ChatInput,
    )
    .option(
        StringBuilder::new("domain".into(), "Select the domain here".into())
            .required(true)
            .choices(domain_options),
    )
    .option(
        StringBuilder::new("subdomain".into(), "Funny subdomain to add.".into()).required(false),
    )
    .build()
}
...
pub async fn execute(client: &Client, cmd: &ApplicationCommand, user: Users) -> Result<()> {
    ...
    let data = format!(
        "https://{}{}",
        sub_domain.unwrap_or_else(|| "".to_owned()),
        domain
    );
    println!("{}", &data);
    ...
    let embed = create_embed()
        .title("Embed")
        .description(format!("Updated your domain to {}", data))
        .build()?;
