...
use ascella_util::Error as AscellaError;
use futures::future;
use governor::{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        let ip: IpAddr = req
            ...
            .map(|r| {
                r.to_str()
                    .unwrap()
                    .parse()
                    .unwrap_or_else(|_| "127.0.0.0".parse().unwrap())
            })
            .unwrap_or_else(|| {
        ...
        match self.limiter.check_key(&ip) {
            ...
            Err(negative) => {
                ...
                //     .body(body.to_owned());
                let response = AscellaError::RateLimit {
                    message: body.clone(),
                };
                future::Either::Left(future::err(
                    error::InternalError::from_response(body, response.error_response()).into(),
                ))
            }
        }
    }
