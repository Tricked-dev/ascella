...
impl Application for AscellaDesktop {
  ...
  fn update(&mut self, message: Message) -> Command<Self::Message> {
    match message {
      ...
      Message::NewConfig => {
        let path = FileDialog::new().add_filter("SXCU File", &["sxcu", "json"]).show_open_single_file().unwrap();
  ...
  fn view(&mut self) -> Element<'_, Self::Message> {
    let darkmode = Toggler::new(self.toggler_value, String::from("Darkmode"), Message::TogglerToggled)
      .width(Length::Shrink)
      .spacing(10)
      .style(self.theme);
