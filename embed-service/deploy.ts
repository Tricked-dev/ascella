...
const BACKEND_URL = Deno.env.get("BACKEND_URL") || "https://ascella.wtf";
const FRONTEND_URL = Deno.env.get("FRONTEND_URL") || "https://ascella.host";
  ...
  [
    ...
    async (req: AugmentedRequest) => {
      ...
      if (req.headers.get("user-agent")?.toLowerCase().includes("discord")) {
        let r = await fetch(
          `${BACKEND_URL}/v2/ascella/view/${name}/stats`,
        );
        if (r.ok) {
          let rson = await r.json();
          if (
            !rson.embed.color && !rson.embed.title && !rson.embed.description
          ) {
            return new Response(undefined, {
              status: 301,
              headers: new Headers({
                ["location"]: `${BACKEND_URL}/v2/ascella/view/${name}`,
              }),
            });
          }
...
<head>
  ...
  <meta property="og:image" content="${`${BACKEND_URL}/v2/ascella/view/${name}`}">
  <meta property="twitter:card" content="summary_large_image">
</head>
<body>
  <img src="${`${BACKEND_URL}/v2/ascella/view/${name}`}">
</body>
</html>`,
            {
              headers: {
