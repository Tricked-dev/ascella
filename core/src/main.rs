...
use ascella_http::set_endpoints;
...
async fn init() -> std::io::Result<()> {
    ...
    spec.info = Info {
        ...
        contact: Some(Contact {name:Some("Tricked".into()), url:Some("https://tricked.pro".into()), email:Some("tricked@tricked.pro".into())}),
        license:Some(License {name:Some("AGPL-3.0".into()), url:Some("https://github.com/Tricked-dev/ascella/blob/master/LICENSE".into()) }),
        ..Default::default()
    };
